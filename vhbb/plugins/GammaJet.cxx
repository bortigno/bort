...
void GammaJet::analyze(const edm::Event& iEvent, const edm::EventSetup& setup)
{
  ...
  //photon ID
  if( not patPhoton.at(0).photonID("PhotonCutBasedIDTight") )
    return void();

  // gluon Candidate
  reco::CompositeCandidate gluonCandidate;
  gluonCandidate.addDaughter( *v_akt5pfj.at(0) );
  ...
  double photonCandidate_pt = photonCandidate.p4().Pt();
  double photonGluon_deltaPhi = Geom::deltaPhi(gluonCandidate.p4(), photonCandidate.p4());
  ...
  double leadingBTag = leadingJet->bDiscriminator("combinedSecondaryVertexBJetTags");

  if( ( (leadingBTag > minBTagging) and (secondLeadingBTag > minBTagging) )
      and ( (leadingBTag > maxBTagging) or (secondLeadingBTag > maxBTagging) ) )
    BTag = true;

  histocontainer_["h_minTagger"]->Fill(  std::min(secondLeadingBTag, leadingBTag) );
  histocontainer_["h_maxTagger"]->Fill(  std::max(secondLeadingBTag, leadingBTag) );

  // Event selection
  if( photonCandidate_pt < 155
      or TMath::Abs(photonGluon_deltaPhi) < 2.75
      or BTag == false )
    return void();  
  ...
  histocontainer_2["h2_LowerDeltaTheta_invmassJet"]->Fill(leadingDeltaTheta, gluonCandidate.p4().M());
  ...
  if(TMath::Abs(otherJet->p4().Eta()) > 0.6 and TMath::Abs(otherJet->p4().Eta() < 0.8))
...
bool GammaJet::muonJetCleaning(const pat::Jet& tmpJet, const std::vector<pat::Muon>& muCol, double deltaPtcut){
  bool clean = false;
//   double deltaR = 1e10;
  double deltaPt = 1e10;
...
double GammaJet::getAnglePhiEtaPlane( const pat::Jet *leadingJet , const pat::Jet *secondLeadingJet ){
  double angle = 1e10;
...
double GammaJet::getDeltaR( const pat::Jet * leadingJet, const pat::Jet * secondLeadingJet ){
  double deltaR = 1e10;
...
double GammaJet::getDeltaR( TLorentzVector TLV, const pat::Jet * patJet ){
  double deltaR = 1e10;
  TVector3 JetVector;
...
double GammaJet::getDeltaEta( const pat::Jet * leadingJet, const pat::Jet * secondLeadingJet ){
...
double GammaJet::getDeltaPhi( const pat::Jet* leadingJet, const pat::Jet* secondLeadingJet){
...
double GammaJet::getPtAsymmetry(const pat::Jet* leadingJet, const pat::Jet* secondLeadingJet ){
...
unsigned int GammaJet::getAssociatedB( std::vector<TLorentzVector> bHadron_vector, const pat::Jet* leadingJet){
...
TVector2 GammaJet::getTvect( const pat::Jet* patJet, TLorentzVector b ){
...
TVector2 GammaJet::getTvect( const pat::Jet* patJet ){
...
double GammaJet::getDeltaTheta( const pat::Jet* j1, const pat::Jet* j2, TLorentzVector b1, TLorentzVector b2 ){
...
//without b vertex information
double GammaJet::getDeltaTheta( const pat::Jet* j1, const pat::Jet* j2 ){
...
TVector2 GammaJet::getBBdir( const pat::Jet* j1, const pat::Jet* j2){
...
inline const pat::Jet* GammaJet::whichJet(const pat::Jet *j1, const pat::Jet *j2){
...
inline const pat::Jet* GammaJet::whichOtherJet(const pat::Jet *j1, const pat::Jet *j2){
...
void GammaJet::beginJob()
{
  ...
  histocontainer_2["h2_pfcEtaPhi"]=fs->make<TH2D>("h2_pfcEtaPhi","h2_pfcEtaPhi", bin_eta, min_eta, max_eta, bin_angle, min_angle, max_angle );
  ...
  histocontainer_2["h2_BBdEtadPhi"]=fs->make<TH2D>("h2_BBdEtadPhi","h2_BBdEtadPhi", bin_eta, min_eta, max_eta, bin_angle, min_angle, max_angle );
  histocontainer_2["h2_BBdirdPhi"]=fs->make<TH2D>("h2_BBdirdPhi","h2_BBdirdPhi", bin_angle, min_angle, max_angle, bin_angle, min_angle, max_angle );
  histocontainer_2["h2_BBdirdEta"]=fs->make<TH2D>("h2_BBdirdEta","h2_BBdirdEta", bin_angle, min_angle, max_angle, bin_eta, min_eta, max_eta );
