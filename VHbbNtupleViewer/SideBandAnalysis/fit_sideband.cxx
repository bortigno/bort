...
int main(int argc, char **argv){
  ...
  //std::string s_suffix_Zbb_SB = "SystFUP$";    
  //std::string s_suffix_Zlight_SB = "SystFUP$";
  std::string s_suffix_Zlight_SB = "$";
  std::string s_suffix_ttbar_SB = "$";
  std::string s_region_Zbb_SB = "Zbb"; // Zbb sideband
  std::string s_var_Zbb_SB = "MET_et"; // 
  std::string s_region_ttbar_SB = "TTbar";
  std::string s_var_ttbar_SB = "ZH_dPhi"; // one addjet required  
  std::string s_region_Zlight_SB = "Zlight";
  std::string s_var_Zlight_SB = "SimpleJet1_bTag";

  if(debug_)
    std::cout << " fillinf the fit info " << std::endl;

  fitInfos.push_back( new fitInfo(s_region_Zbb_SB,s_var_Zbb_SB,s_prefix,s_sysprefix,s_suffix_Zbb_SB,s_channel,0,100) );
  fitInfos.push_back( new fitInfo(s_region_ttbar_SB,s_var_ttbar_SB,s_prefix,s_sysprefix,s_suffix_ttbar_SB,s_channel,0,5) );
  fitInfos.push_back( new fitInfo(s_region_Zlight_SB,s_var_Zlight_SB,s_prefix,s_sysprefix,s_suffix_Zlight_SB,s_channel,0,1) );

  std::string signalString = s_sysprefix+"SideBandRegion"+s_channel+"/HiggsMass"+s_sysprefix+"SideBandRegion"+s_channel+"$";
...
// 	      if(!fitSys){ // if you want to fit data
// 		if(s[j].data){
...
// 	      }
  ...
  for(int i=0; i<fitInfos.size(); ++i)
    fitInfos.at(i)->create_variable(templateNames,fixedTemplateNames,f_vars); //
