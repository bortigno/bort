  ...
  for(size_t i = 0 ; i < names.size() ; i++) 
    {
      ...
      if(!n.Contains(TRegexp("ZSVRegionHZcom"))) continue;

      if(n.Contains(TRegexp("ZSVRegionHZcom")))
	process = "Z(l^{+}l^{-})H(b#bar{b})";
      if(n.Contains(TRegexp("ZSVRegionHZmm")))
	process = "Z(#mu^{+}#mu^{-})H(b#bar{b})";
      if(n.Contains(TRegexp("ZSVRegionHZee")))
