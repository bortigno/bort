...
class ZbbXsecRegionHZee: public CutSample {
  std::string name() {return "ZbbXsecRegionHZee";};
 public:
  Bool_t pass(ntupleReader &p){
    return( 
	   (p.hbhe & p.hJet_pt[0] > 20. & p.hJet_pt[1] > 20. & p.EVENT_json & p.V_mass > 75. & p.V_mass < 105 & ((p.hJet_csv[0] > 0.898 & p.hJet_csv[1] > 0.244) | (p.hJet_csv[0] > 0.244 & p.hJet_csv[1] > 0.898)) & p.Vtype == 1 & ( p.triggerFlags[5] | p.triggerFlags[6] ))
 );    
  }
  Bool_t pass(ntupleReader &p, Sample &sample){
    bool sampleCut = false;
...
class ZbbXsecLightRegionHZee: public CutSample {
  std::string name() {return "ZbbXsecLightRegionHZee";};
 public:
  Bool_t pass(ntupleReader &p){
    return( 
	   (p.hbhe & p.hJet_pt[0] > 20. & p.hJet_pt[1] > 20. & p.EVENT_json & p.V_mass > 75. & p.V_mass < 105 & ((p.hJet_csv[0] > 0.5 & p.hJet_csv[1] > 0.244) | (p.hJet_csv[0] > 0.244 & p.hJet_csv[1] > 0.5)) & p.Vtype == 1 & ( p.triggerFlags[5] | p.triggerFlags[6] ))
 );    
  }
  Bool_t pass(ntupleReader &p, Sample &sample){
    bool sampleCut = false;
